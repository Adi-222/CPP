...
int main()
{
       parse(); // Assuming parse() function populates necessary data structures

    // Iterating through each algorithm
    for (int idx = 0; idx < (int)algorithms.size(); idx++) {
        clear_timeline(); // Clear the timeline
        execute_algorithm(algorithms[idx].first, algorithms[idx].second, operation); // Execute the algorithm
        if (operation == TRACE) // If operation is trace
            printTimeline(idx); // Print the timeline
        else if (operation == SHOW_STATISTICS) // If operation is to show statistics
            printStats(idx); // Print statistics
        cout << "\n"; // Print a newline
    }
